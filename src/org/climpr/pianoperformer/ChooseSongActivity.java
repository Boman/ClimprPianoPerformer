...
/**
 * @class IconArrayAdapter The ListAdapter for displaying the list of songs.
 *        Similar to the array adapter, but adds a NotePair icon to the left
 *        side of each item displayed.
 */
class IconArrayAdapter<T> extends ArrayAdapter<T> {
	...
	/**
	 * Create a view for displaying a song in the ListView. The view consists of
	 * a Note Pair icon on the left-side, and the name of the song.
	 */
	@Override
...
/**
 * @class ChooseSongActivity The ChooseSongActivity class is used to display a
 *        list of songs to choose from. The list is created from the songs
 *        shipped with MidiSheetMusic (in the assets directory), and also by
 *        searching for midi files in the internal/external device storage.
 * 
 *        When a song is chosen, this calls the SheetMusicAcitivty, passing the
 *        raw midi byte[] data as a parameter in the Intent.
 */
public class ChooseSongActivity extends ListActivity implements TextWatcher {
	...
	/**
	 * Create the choose song activity. Add all the midi file resources into the
	 * song list. Search for additional midi files in the internal/external
	 * storage, and also add those to the songlist.
	 */
	@Override
	...
	/**
	 * Look for midi files (with mime-type audio/midi) in the internal/external
	 * storage. Add them to the songlist.
	 */
	private void loadMidiFilesFromStorage(Uri content_uri) {
		ContentResolver resolver = getContentResolver();
		String columns[] = { MediaStore.Audio.Media._ID, MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.MIME_TYPE };
	...
	/**
	 * When a song is clicked on, start a SheetMusicActivity. Read the raw
	 * byte[] data of the midi file. Pass the raw byte[] data as a parameter in
	 * the Intent. Pass the midi file Title as a parameter in the Intent.
	 */
	@Override
	protected void onListItemClick(ListView parent, View view, int position, long id) {
		...
		intent.putExtra(PianoActivity.MidiDataID, data);
		intent.putExtra(PianoActivity.MidiTitleID, file.toString());
		startActivity(intent);
	}

	/**
	 * As text is entered in the filter box, filter the list of midi songs to
	 * display.
	 */
	@Override
