 ...
 */
public class MidiSettings implements Serializable {
	...
	public MidiSettings(MidiFile midifile) {
		showPiano = true;
	...
	public static MidiSettings fromJson(String jsonString) {
		if (jsonString == null) {
			return null;
		}
		MidiSettings options = new MidiSettings();
		try {
	...
	public void merge(MidiSettings saved) {
		...
		if (saved.time != null) {
			time = new TimeSignature(saved.time.getNumerator(), saved.time.getDenominator(), saved.time.getQuarter(),
					saved.time.getTempo());
		}
